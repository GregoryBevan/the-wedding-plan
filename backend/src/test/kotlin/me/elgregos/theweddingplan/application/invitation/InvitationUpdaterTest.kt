...
class InvitationUpdaterTest {
    ...
    fun `should update invitation when all guests are active and available`() {
        ...
        every { invitations.findById(brideFamily.invitationId) } returns brideFamilyInvitation
    ...
    fun `should reject update when invitation does not exist`() {
        every { invitations.findById(brideFamily.invitationId) } returns null
    ...
    fun `should reject update when at least one guest is already assigned to another invitation`() {
        every { invitations.findById(mixedGuests.invitationId) } returns brideFamilyInvitation
        ...
        assertThat(invitationUpdater.update(mixedGuests)).isEqualTo(UpdateInvitationResult.AlreadyAssignedGuests(setOf(janeDoe.id)))
        verify(exactly = 0) { invitations.update(any()) }
    }
    ...
    fun `should return already assigned guests when update fails due to concurrent unique constraint`() {
        every { invitations.findById(mixedGuests.invitationId) } returns brideFamilyInvitation
        every { guests.findByIds(mixedGuests.guestIds) } returns setOf(johnDoe, janeDoe)
        every { invitations.findAssignedGuestIds(mixedGuests.guestIds) } returnsMany listOf(setOf(johnDoe.id), mixedGuests.guestIds)
    ...
    fun `should reject update when at least one guest is missing`() {
        every { invitations.findById(missingGuests.invitationId) } returns brideFamilyInvitation
    ...
    fun `should return missing guests when guest list is empty`() {
        every { invitations.findById(noGuest.invitationId) } returns friendsInvitation
    ...
    fun `should return not found when invitation disappears during update`() {
        ...
        every { invitations.findById(command.invitationId) } returns brideFamilyInvitation
